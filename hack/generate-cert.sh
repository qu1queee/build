...
apiVersion: v1
data:
  tls.crt: $(cat /tmp/server-cert.pem | base64 | tr -d '\n')
  tls.key: $(cat /tmp/server-key.pem | base64 | tr -d '\n')
kind: Secret
metadata:
...
echo "[INFO] Applying CABundle into 600-webhook.yaml"
sed -i '' "s/CA_BUNDLE/${CA}/" $DIR/deploy/600-webhook.yaml
