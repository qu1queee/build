...
func GenerateSA(ctx context.Context, client client.Client, build *buildv1alpha1.Build, buildRun *buildv1alpha1.BuildRun) (*corev1.ServiceAccount, error) {
	mountSAToken := false

	// populate the object with well known required fields
	generatedSA := &corev1.ServiceAccount{
	...
	ctxlog.Debug(ctx, "automatic generation of service account", namespace, generatedSA.Namespace, name, generatedSA.Name)

	// if the service account already exists, do nothing and just return it
	err := client.Get(ctx,
	...
	if apierrors.IsNotFound(err) {
		if err := client.Create(ctx, generatedSA); err != nil {
			return nil, err
		}
		ctxlog.Info(ctx, "created serviceAccount for BuildRun", namespace, buildRun.Namespace, name, generatedSA.Name, "BuildRun", buildRun.Name)
		// add the secrets references into the new sa
		ApplyCredentials(ctx, build, generatedSA)
		return generatedSA, nil
	}
	ctxlog.Info(ctx, "serviceAccount for BuildRun already exists", namespace, buildRun.Namespace, name, generatedSA.Name, "BuildRun", buildRun.Name)
	return generatedSA, nil
}
...
func RetrieveServiceAccount(ctx context.Context, client client.Client, build *buildv1alpha1.Build, buildRun *buildv1alpha1.BuildRun) (*corev1.ServiceAccount, error) {
	...
	if buildRun.Spec.ServiceAccount != nil {
		if buildRun.Spec.ServiceAccount.Name != nil {
			...
			if err != nil && !apierrors.IsNotFound(err) {
				...
				return nil, UpdateConditionWithFalseStatus(ctx, client, buildRun, fmt.Errorf("service account %s not found", serviceAccountName).Error(), ConditionServiceAccountNotFound)
			}
		} else {
