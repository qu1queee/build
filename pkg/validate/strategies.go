...
func (s Strategy) validateBuildParams(parameters []build.Parameter) error {
	...
	if len(s.Build.Spec.Params) == 0 {
		return nil
	}

	// Check that the Build param is defined in the strategies parameters
	s.validateParamsInStrategies(s.Build.Spec.Params, parameters)

	// Check that the Build param is not a restriced shipwright one
...
func (s Strategy) validateParamsNamesDefinition() {
	...
	for _, p := range s.Build.Spec.Params {
...
func (s Strategy) validateParamsInStrategies(params []build.Param, parameters []build.Parameter) {
